...
def  SIR_filter(model,prior,state_dim,ens_size,nanl,tanl,obs,Q,threshold,tuning):
    ...
    prior_S = prior
    prior_resample = resample_count
    [analysis,weights,resampe_count] = SIR_update(weights,threshold,tuning,resample_count,
                                             obs[i,:],Q,prior,ens_size,state_dim)
    post_S = analysis        
    A_i = A + str(i+1)
    p_series[A_i] = {'prior':prior_S,'prior_weight':prior_W,'post':post_S,
                     'post_weight':weights, 'prior_resample':prior_resample, 
                     'post_resample':resample_count}
