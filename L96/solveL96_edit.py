...
def ComputeLorentzDrift(xin, tin):
    ...
    nextk = np.concatenate((xin[1:],[xin[0]]),axis=1)
    # index for the previous grid point
    prevk = np.concatenate(([xin[-1]],xin[0:-1]),axis=1)
    # index for previous to previous grid point
    prevkk = np.concatenate((xin[-2:],xin[0:-2]),axis=1)
    drift = (nextk - prevkk) * prevk - xin + F;
...
plt.figure(1)
plt.plot(tgrid,xsol[:,0])
plt.plot(ttmp,xrk4[0,:])
